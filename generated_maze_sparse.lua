...
function api:init(settings)
    ...
    -- Load the file data
    local ents_file = io.open("demos/map_generation/sparse_mazes/maze_ents_" .. tostring(i) .. ".txt", "r")
    local ents = ents_file:read("*a")
    local vars_file = io.open("demos/map_generation/sparse_mazes/maze_vars_" .. tostring(i) .. ".txt", "r")
